...
// ----------------------------------------------------------
    ...
    IEnumerator WaveTimer()
    {
        while (true)
        {
            ...
            // 追加
            if (currentWaveIndex % bossWaveCount == 0 && currentWaveIndex != 0)
            {
                // ボス出現
                if (!bossSpawnController.IsBossSpawned())       // ボスが出現していない場合
                {
                    bossSpawnController.SpawnBoss();            // ボスを出現させる
                    waveTimerSlider.maxValue = 1;
                    waveTimerSlider.value = 1;
                }
            }

            if (!bossSpawnController.IsBossSpawned())           // 追加
            {
                // Waveデータをルールに基づいて生成
                currentWaveData = WaveGenerator.GenerateWave(currentWaveIndex + 1, waveRule);
            ...
            {
                yield return new WaitUntil(() => FieldOnEnemiesCheck());
                yield return new WaitUntil(() => !bossSpawnController.IsBossSpawned());     // 追加
            }
